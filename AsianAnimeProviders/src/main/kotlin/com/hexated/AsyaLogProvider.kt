...
class AsyaLogProvider : MainAPI() {
    ...
    private val fallbackDomains = listOf(
        "https://asyalog.co",
        "https://asyawatch.com",
        "https://asyafanatiklerim.com"
    )

    private suspend fun getUrl(): String {
        return DynamicDomainHelper.getActiveDomain("asyalog", fallbackDomains)
    }

    override val mainPage = mainPageOf(
        "" to "Son Eklenen Asya Dizileri",
        "kore-dizileri" to "Kore Dizileri",
        "cin-dizileri" to "Çin Dizileri",
        "japon-dizileri" to "Japon Dizileri"
    )
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = doc.select("div.poster, div.drama-card, article, div.item").mapNotNull {
            it.toSearchResult()
        }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("div.poster, div.drama-card, article, div.search-result").mapNotNull {
            it.toSearchResult()
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val title = doc.selectFirst("h1, .entry-title, .title")?.text()?.trim() ?: "Bilinmeyen Asya Dizisi"
