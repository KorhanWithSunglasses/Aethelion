...
class DizillaProvider : MainAPI() {
    ...
    private suspend fun getUrl(): String {
        return DynamicDomainHelper.getActiveDomain("dizilla", fallbackDomains)
    }

    override val mainPage = mainPageOf(
        "" to "Son Eklenen Bölümler",
        "diziler" to "Popüler Diziler",
        "tur/trend-diziler" to "Trend Diziler"
    )
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = doc.select("div.poster, div.dizi-kutu, article").mapNotNull {
            it.toSearchResult()
        }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("div.poster, div.dizi-kutu, article, div.search-result").mapNotNull {
            it.toSearchResult()
        }
    }
